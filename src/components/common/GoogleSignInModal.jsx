...
export default function GoogleSignInModal({ isOpen, onClose, onSuccess }) {
  ...
  return (
    <div className="position-fixed top-0 start-0 w-100 h-100 z-3 d-flex align-items-center justify-content-center bg-dark bg-opacity-75 p-3 backdrop-blur">
      <div className="card border-0 rounded-5 shadow-2xl w-100 overflow-hidden" style={{ maxWidth: '440px' }}>
        <div className="card-body p-4 position-relative">
          {/* Close Button */}
          <button
            onClick={onClose}
            className="btn btn-sm btn-light rounded-circle position-absolute top-0 end-0 m-3 p-1 border shadow-xs"
          >
          ...
          <div className="text-center mb-4">
            <div className="rounded-4 bg-white d-flex align-items-center justify-content-center mx-auto mb-3 border shadow-sm" style={{ width: '52px', height: '52px' }}>
              <svg className="w-6 h-6" viewBox="0 0 24 24" style={{ width: '28px', height: '28px' }}>
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  <div className="fw-black text-dark fs-6">
                    {currentGoogleAccount.name}
                  </div>
                  <div className="text-muted fw-bold" style={{ fontSize: '0.78rem' }}>
                    {currentGoogleAccount.email}
                  </div>
                </div>
              </div>
          ...
          <div className="border-top pt-3">
            <form onSubmit={handleCustomGoogleSubmit} className="d-flex flex-column gap-2">
              <label className="form-label text-muted fw-bold text-uppercase" style={{ fontSize: '0.68rem', letterSpacing: '0.04em' }}>
              ...
              <input
                ...
                className="form-control form-control-lg rounded-3 fs-6 px-3"
              />
              <button
                type="submit"
